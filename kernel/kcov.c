...
pr_err("! kcov_remote_area_get: %u\n", size);

	list_for_each(pos, &kcov_remote_areas) {
		area = list_entry(pos, struct kcov_remote_area, list);
		if (area->size == size) {
			list_del(&area->list);
pr_err("! kcov_remote_area_get = %px\n", area);
			return area;
		}
	}
pr_err("! kcov_remote_area_get = NULL\n");
	return NULL;
}

static void kcov_remote_area_put(struct kcov_remote_area *area, unsigned size)
{
pr_err("! kcov_remote_area_put: %px %u\n", area, size);
...
{
pr_err("! kcov_start: t = %px, size = %u, area = %px\n", t, size, area);
...
pr_err("! kcov_remote_reset\n");

	spin_lock(&kcov_remote_lock);
	hash_for_each_safe(kcov_remote_map, bkt, tmp, remote, hnode) {
		if (remote->kcov != kcov)
			continue;
pr_err("! kcov_remote_reset: removing %llx\n", remote->handle);
...
pr_err("! kcov_task_exit: t = %px, kcov->t = %px\n", t, kcov->t);
...
pr_err("! kcov_remote_start: handle = %llx\n", handle);
	...
	if (!remote) {
pr_err("! kcov_remote_start: no remote => fail");
		spin_unlock(&kcov_remote_lock);
		return;
	}
	...
	if (!area) {
pr_err("! kcov_remote_start: vmalloc(%u)\n", size);
		area = vmalloc(size * sizeof(unsigned long));
		if (!area) {
pr_err("! kcov_remote_start: no area => fail");
...
pr_err("! kcov_remote_start = success\n");
}
...
pr_err("! kcov_move_area: %px %u <= %px %lu\n",
...
pr_err("! kcov_remote_stop\n");

	if (!kcov)
{
pr_err("! kcov_remote_stop: no kcov => fail\n");
		return;
}
	 ...
	 */
pr_err("! kcov_remote_stop: move if: %d == %d && %d\n",
...
pr_err("! kcov_remote_stop = success\n");
}
